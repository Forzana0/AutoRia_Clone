...
const NewPassword: React.FC<NewPasswordProps> = ({ onBack, onSuccess, email, code }) => {
    ...
    const handleSubmit = async (ev: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            await api.post('/api/Accounts/ResetPasswordWithCode', {
                email,
                code,
            ...
            const msg = err?.response?.data?.Message || err?.response?.data || 'Помилка зміни паролю';
            setErrors({ general: typeof msg === 'string' ? msg : 'Спробуйте ще раз' });
        } finally {
            setLoading(false);
        }
    };
