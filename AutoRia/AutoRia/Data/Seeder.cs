    ...
    public static class Seeder
    {
        public static async void SeedData(this IApplicationBuilder app)
        {
                       ...
                       .GetRequiredService<IServiceScopeFactory>().CreateScope())
            {
                ...
                {
                    foreach (var roleName in configuration.GetSection("Roles").GetChildren())
                    {
                ...
                //{
                //    var faker = new Faker();W
                //    var users = new List<UserEntity>();


                //    // Генеруємо 10 випадкових користувачів
                //    for (int i = 0; i < 10; i++)
                //    {
                //        var city = await context.Cities.OrderBy(r => Guid.NewGuid()).FirstOrDefaultAsync();
                ...
                //        } while (users.Any(u => u.UserName == userName)); // Ensure unique userName

                //        string email;
                //        do
                //        {
                //            email = faker.Internet.Email();
                //        } while (users.Any(u => u.Email == email)); // Ensure unique email
                ...
                //            Email = email,
                //            UserName = userName,
                //            Photo = await imageService.SaveImageAsync(base64Image), // Додати реальний базовий64 код зображення
                //            PhoneNumber = faker.Phone.PhoneNumber(),
                //            Rating = faker.Random.Decimal(0, 5).ToString(),
                //            City = city.Name, // Випадкове місто
                //            Region = city.Region.Name, // Випадковий регіон
                //        };
                ...
                //    // Додаємо всіх користувачів до контексту за один раз
                //    //await context.Users.AddRangeAsync(users);
                //    //await context.SaveChangesAsync();

                //    // Сідаємо машини для кожного користувача
                //    foreach (var user in users)
                //    {
                //        int numberOfCars = faker.Random.Int(1, 5); // Випадкова кількість машин для користувача
                //        var fakeCars = new List<CarEntity>();

                //        for (int i = 0; i < numberOfCars; i++)
                //        {
                //            var carModel = await context.Models.Include(m => m.CarBrand).OrderBy(r => Guid.NewGuid()).FirstOrDefaultAsync();
                ...
                //                City = await context.Cities.Where(c => c.Name == user.City).FirstOrDefaultAsync(), // Використовуємо місто користувача
                ...
                //        // Прив'язка автомобілів до користувача
                //        foreach (var car in fakeCars)
                //        {
                ...
                if (await context.Users.CountAsync() == 0)
                {
                    ...
                    for (int i = 0; i < 11; i++)
                    {
                        ...
                        {
                            userName = faker.Internet.UserName();
                        } while (context.Users.Any(u => u.UserName == userName)); // Ensure unique userName

                        string email;
                        do
                        {
                            email = faker.Internet.Email();
                        } while (context.Users.Any(u => u.Email == email)); // Ensure unique email
                    ...
                    IdentityResult result2 = await userManager.CreateAsync(
                       user2,
                    ...
                    result2 = await userManager.AddToRoleAsync(user2, "User");
                ...
                if (await context.Users.CountAsync() == 0)
                {
                    ...
                    foreach (var user in await context.Users.ToArrayAsync())
                    {
                        int numberOfCars = faker.Random.Int(1, 5); // Випадкова кількість машин для користувача
                        var fakeCars = new List<CarEntity>();
                        int rId = faker.Random.Int(context.Models.First().Id, context.Models.First().Id + context.Models.Count());
                        for (int i = 0; i < numberOfCars; i++)
                        {
                            var carModel = await context.Models.Where(cm => cm.Id == rId).Include(m => m.CarBrand).FirstOrDefaultAsync();
                            //var bodyType = await context.BodyTypes.OrderBy(r => Guid.NewGuid()).FirstOrDefaultAsync();
                            ...
                            var car = new CarEntity
                            {
                                ...
                                DateCreated = DateTime.UtcNow.AddDays(faker.Random.Int(-10, -1)),
                                TransportType = await context.TransportTypes.OrderBy(r => Guid.NewGuid()).FirstOrDefaultAsync(),
                                BodyType = await context.BodyTypes.OrderBy(r => Guid.NewGuid()).FirstOrDefaultAsync(),
                                TransmissionType = await context.TransmissionTypes.OrderBy(r => Guid.NewGuid()).FirstOrDefaultAsync(),
                                NumberOfSeats = await context.numbersOfSeats.OrderBy(r => Guid.NewGuid()).FirstOrDefaultAsync(),
                                FuelTypes = await context.FuelTypes.OrderBy(r => Guid.NewGuid()).FirstOrDefaultAsync(),
                                EngineVolume = await context.EngineVolumes.OrderBy(r => Guid.NewGuid()).FirstOrDefaultAsync(),
